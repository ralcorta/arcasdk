...
describeOrSkip(
  "WSFE errores — TA manual (consumidor npm)",
  () => {
    let arca: Arca;
        ...
        const msg =
          e instanceof Error
            ...
            : typeof e === "string"
              ? e
              : "unknown";
        throw new Error(`Setup WSFE errores TA manual (consumidor): ${msg}`);
      }
    });
...
if (!enableIntegration) {
  console.info("Omitiendo tests WSFE (errores TA manual): ENABLE_INTEGRATION_TESTS=true");
}
