...
describe("GenericRepository", () => {
  ...
  describe("call", () => {
    it("should call generic method with SOAP 1.1", async () => {
      ...
      const result = await repository.call(ServiceNamesEnum.WSFE, "testMethod", {
        param: "value",
      });
    ...
    it("should throw error if method does not exist", async () => {
      ...
      await expect(
        repository.call(ServiceNamesEnum.WSFE, "nonExistentMethod", {}),
      ).rejects.toThrow(
        `Method nonExistentMethod not found on service ${ServiceNamesEnum.WSFE}`,
      );
    });
  });
});
