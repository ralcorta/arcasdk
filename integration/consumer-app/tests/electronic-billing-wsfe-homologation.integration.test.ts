...
import {
  ...
  expectCaeaNoMovementHomologation,
  expectCaeaUsageHomologation,
  expectNonEmptyArray,
...
describeOrSkip("WSFE homologación — facturación electrónica (consumidor npm)", () => {
  let arca: Arca;
  ...
  describe("FEParamGetPtosVenta", () => {
    it("getSalesPoints: lista de PV o 602 sin puntos en padrón WSFE", async () => {
      ...
      if (list.length > 0) {
        ...
        expect(errs[0]!.msg).toMatch(/FEParamGetPtosVenta|PtosVenta|Sin Resultados/i);
      }
    });
  });
  ...
  describe("FECompUltimoAutorizado / FECompConsultar", () => {
    ...
    it("getVoucherInfo sin errores si ya hay comprobante tipo 1 en el PV", async () => {
      ...
      expectNonEmptyString("codAutorizacion", String(info!.codAutorizacion ?? ""));
    });
  });
  ...
  describe("FECAESolicitar — Facturas A, B y C (homologación)", () => {
    it("Factura A (CbteTipo 1): RI + IVA 21 %", async () => {
      ...
      const { resultado, fecha } = await createVoucherHomologacionWithRetry(
        ...
        1,
        (n, f) =>
          buildFacturaA(puntoVenta, docNro, condIva, n, f),
      );
    ...
    it("Factura B (CbteTipo 6): gravado + IVA 21 %", async () => {
      ...
      const { resultado, fecha } = await createVoucherHomologacionWithRetry(
        ...
        6,
        (n, f) =>
          buildFacturaB(puntoVenta, docNro, condIva, n, f),
      );
